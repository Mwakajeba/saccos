...
class ReceiptVoucherController extends Controller
{
    ...
    /**
     * Resolve receipt voucher from encoded ID (same pattern as Payment Voucher).
     * Tries HashIdHelper first, then Vinkla Hashids for backward compatibility with old URLs.
     */
    protected function resolveReceiptVoucher($encodedIdOrInstance)
    {
        if ($encodedIdOrInstance instanceof Receipt) {
            return $encodedIdOrInstance;
        }
        $encoded = (string) $encodedIdOrInstance;
        // URL safety: plus can become space in query/path
        $encoded = str_replace(' ', '+', $encoded);
        $decodedId = HashIdHelper::decode($encoded);
        if ($decodedId !== null && $decodedId > 0) {
            $receipt = Receipt::find($decodedId);
            if ($receipt) {
                return $receipt;
            }
        }
        // Fallback: old receipt URLs may have used Vinkla Hashids
        try {
            $decoded = Hashids::decode($encoded);
            if (!empty($decoded) && isset($decoded[0])) {
                return Receipt::find($decoded[0]);
            }
        } catch (\Throwable $e) {
            // ignore
        }
    ...
    public function data()
    {
        ...
        return DataTables::of($receipts)
            ...
            ->addColumn('reference_link', function ($receipt) {
                return '<a href="' . route('accounting.receipt-vouchers.show', $receipt->hash_id) . '" class="text-primary">RCP-' . $receipt->id . '</a>';
            })
            ...
            ->addColumn('actions', function ($receipt) {
                ...
                $actions .= '<a href="' . route('accounting.receipt-vouchers.show', $receipt->hash_id) . '" class="btn btn-sm btn-info me-1" title="View"><i class="bx bx-show"></i></a>';
                $actions .= '<a href="' . route('accounting.receipt-vouchers.edit', $receipt->hash_id) . '" class="btn btn-sm btn-primary me-1" title="Edit"><i class="bx bx-edit"></i></a>';
    ...
    public function store(Request $request)
    {
        ...
        try {
            return $this->runTransaction(function () use ($request) {
                ...
                $whtEnabled = ($request->input('wht_enabled') === '1' || $request->input('wht_enabled') === 1);
                ...
                // Refresh receipt so GL creation sees saved wht_amount, vat_amount, base_amount, etc.
                $receipt->refresh();
                ...
                if ($request->ajax() || $request->wantsJson()) {
                    $redirectUrl = route('accounting.receipt-vouchers.show', $receipt->hash_id);
                ...
                return redirect()->route('accounting.receipt-vouchers.show', $receipt->hash_id)
                    ->with('success', 'Receipt voucher created successfully.');
            });
        } catch (\Exception $e) {
    ...
    {
        $receiptVoucher = $this->resolveReceiptVoucher($encodedId);
        if (!$receiptVoucher) {
    ...
    public function update(Request $request, $encodedId)
    {
        ...
        try {
            return $this->runTransaction(function () use ($request, $receiptVoucher) {
                ...
                // Get WHT and VAT settings from request (use wht_enabled from request; hidden input ensures it's always sent on edit)
                $whtEnabled = ($request->input('wht_enabled') === '1' || $request->input('wht_enabled') === 1);
                $whtTreatment = $whtEnabled ? ($request->wht_treatment ?? 'EXCLUSIVE') : 'NONE';
                $whtRate = $whtEnabled ? (float) ($request->wht_rate ?? 0) : 0;
                ...
                // Use Receipt model's createGlTransactions method which handles WHT correctly
                $receiptVoucher->refresh(); // Refresh to get updated receipt data (wht_amount, etc.)
                $receiptVoucher->load('receiptItems'); // Ensure new items (with WHT) are loaded for GL
                $receiptVoucher->createGlTransactions();

                return redirect()->route('accounting.receipt-vouchers.show', $receiptVoucher->hash_id)
    ...
    /**
     * Export receipt voucher as PDF (same pattern as Payment Voucher / Account Transfer).
     */
    public function exportPdf($encodedId)
    {
        ...
        try {
            ...
            $pdf = PdfFacade::loadView('accounting.receipt-vouchers.pdf', [
                'receiptVoucher' => $receiptVoucher,
                'invoice' => $invoice
            ]);
            $pdf->setPaper('A4', 'portrait');

            $filename = 'Receipt_Voucher_' . preg_replace('/[^a-zA-Z0-9_-]/', '_', $receiptVoucher->reference ?? 'RCP-' . $receiptVoucher->id) . '_' . date('Y-m-d_H-i-s') . '.pdf';

            // Use the package's download() so Content-Length and Content-Disposition are set correctly
            $response = $pdf->download($filename);

            // If PDF is empty or too small, DomPDF failed – redirect with error
            $content = $response->getContent();
            if ($content === null || strlen($content) < 100) {
                \Log::warning('Receipt voucher PDF empty or too small', ['receipt_id' => $receiptVoucher->id, 'size' => $content ? strlen($content) : 0]);
                return redirect()->route('accounting.receipt-vouchers.index')
                    ->withErrors(['error' => 'PDF could not be generated. Please check receipt data and try again.']);
            }

            return $response;
        } catch (\Throwable $e) {
            \Log::error('Receipt voucher PDF export failed', [
                'encoded_id' => $encodedId,
                'receipt_id' => $receiptVoucher ? $receiptVoucher->id : null,
                'message'   => $e->getMessage(),
                'file'      => $e->getFile(),
                'line'      => $e->getLine(),
            ]);
            return redirect()->route('accounting.receipt-vouchers.index')
                ->withErrors(['error' => 'Failed to export PDF: ' . $e->getMessage()]);
        }
    }
    ...
    {
        try {
            $receipt = $this->resolveReceiptVoucher($id);
            if (!$receipt) {
