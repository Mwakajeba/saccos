...
class LoanWriteoffApprovalSetting extends Model
{
    protected $table = 'loan_writeoff_approval_settings';
     ...
     * Returns 0 if no approval needed.
     */
    public function getRequiredApprovalLevel(float $amount): int
    {
        if ($this->require_approval_for_all) {
            return (int) $this->approval_levels;
        }

        if ($amount <= ($this->auto_approval_limit ?? 0)) {
            return 0;
        }

        return max(1, (int) $this->approval_levels);
    }
