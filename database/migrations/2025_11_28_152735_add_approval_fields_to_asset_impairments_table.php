...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('asset_impairments', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('asset_impairments', 'current_approval_level')) {
                $table->unsignedTinyInteger('current_approval_level')->nullable()->after('status')
                    ->comment('Current approval level being processed');
            }
            
            if (!Schema::hasColumn('asset_impairments', 'submitted_by')) {
                $table->foreignId('submitted_by')->nullable()->after('current_approval_level')
                    ->constrained('users')->onDelete('set null')
                    ->comment('User who submitted for approval');
            }
            
            if (!Schema::hasColumn('asset_impairments', 'submitted_at')) {
                $table->timestamp('submitted_at')->nullable()->after('submitted_by')
                    ->comment('When impairment was submitted for approval');
            }
            
            if (!Schema::hasColumn('asset_impairments', 'rejected_by')) {
                $table->foreignId('rejected_by')->nullable()->after('approved_at')
                    ->constrained('users')->onDelete('set null')
                    ->comment('User who rejected the impairment');
            }
            
            if (!Schema::hasColumn('asset_impairments', 'rejected_at')) {
                $table->timestamp('rejected_at')->nullable()->after('rejected_by')
                    ->comment('When impairment was rejected');
            }
            
            if (!Schema::hasColumn('asset_impairments', 'rejection_reason')) {
