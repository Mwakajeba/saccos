...
@section('title', 'Create Biometric Device')
...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <h6 class="mb-0 text-uppercase">Create Biometric Device</h6>
        <hr />
        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{ route('hr.biometric-devices.store') }}">
                    @csrf
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="device_code" class="form-control @error('device_code') is-invalid @enderror" 
                                   value="{{ old('device_code') }}" required placeholder="e.g., DEVICE-001" />
                            @error('device_code')
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="device_name" class="form-control @error('device_name') is-invalid @enderror" 
                                   value="{{ old('device_name') }}" required placeholder="e.g., Main Entrance Device" />
                            @error('device_name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="device_type" class="form-select @error('device_type') is-invalid @enderror" required>
                                ...
                                <option value="face" {{ old('device_type') == 'face' ? 'selected' : '' }}>Face Recognition</option>
                                <option value="card" {{ old('device_type') == 'card' ? 'selected' : '' }}>Card Reader</option>
                                <option value="palm" {{ old('device_type') == 'palm' ? 'selected' : '' }}>Palm Print</option>
                            </select>
                            @error('device_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="connection_type" class="form-select @error('connection_type') is-invalid @enderror" required>
                                <option value="api" {{ old('connection_type') == 'api' ? 'selected' : '' }}>API (REST)</option>
                                <option value="tcp" {{ old('connection_type') == 'tcp' ? 'selected' : '' }}>TCP/IP</option>
                                <option value="udp" {{ old('connection_type') == 'udp' ? 'selected' : '' }}>UDP</option>
                                <option value="file_import" {{ old('connection_type') == 'file_import' ? 'selected' : '' }}>File Import</option>
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            @error('ip_address')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">Required for TCP/UDP connections</div>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="port" min="1" max="65535" 
                                   ...
                                   value="{{ old('port') }}" placeholder="8080" />
                            @error('port')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="device_model" class="form-control @error('device_model') is-invalid @enderror" 
                                   value="{{ old('device_model') }}" placeholder="e.g., ZKTeco K40" />
                            @error('device_model')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="timezone" class="form-select @error('timezone') is-invalid @enderror" required>
                                <option value="Africa/Dar_es_Salaam" {{ old('timezone', 'Africa/Dar_es_Salaam') == 'Africa/Dar_es_Salaam' ? 'selected' : '' }}>Africa/Dar es Salaam (EAT)</option>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="sync_interval_minutes" min="1" max="1440" 
                                   ...
                                   value="{{ old('sync_interval_minutes', 15) }}" required />
                            @error('sync_interval_minutes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">How often to sync data from device</div>
                        </div>

                        <div class="col-md-12">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="auto_sync" id="auto_sync" 
                                       value="1" {{ old('auto_sync', true) ? 'checked' : '' }}>
                                <label class="form-check-label" for="auto_sync">
                                    Enable Auto Sync
                                </label>
                            </div>
                            <div class="form-text">Automatically sync data at the specified interval</div>
                        </div>
                    ...
                    <div class="d-flex gap-2 mt-4">
                        ...
                        <a href="{{ route('hr.biometric-devices.index') }}" class="btn btn-secondary">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
