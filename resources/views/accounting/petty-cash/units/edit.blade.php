...
@section('title', 'Edit Petty Cash Unit')
...
<div class="page-wrapper">
    <div class="page-content">
        <x-breadcrumbs-with-icons :links="[
            ...
            ['label' => 'Edit Petty Cash Unit', 'url' => '#', 'icon' => 'bx bx-edit']
        ]" />
        
        <h6 class="mb-0 text-uppercase">EDIT PETTY CASH UNIT</h6>
        <hr />

        <div class="card radius-10">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="bx bx-wallet me-2"></i>Edit Petty Cash Unit: {{ $unit->name }}</h5>
            </div>
            <div class="card-body">
                @if($errors->any())
                ...
                <form id="petty-cash-unit-form" action="{{ route('accounting.petty-cash.units.update', $unit->encoded_id) }}" method="POST">
                    @csrf
                    @method('PUT')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select select2-single @error('branch_id') is-invalid @enderror" 
                                        ...
                                        @foreach($branches as $branch)
                                            <option value="{{ $branch->id }}" {{ old('branch_id', $unit->branch_id) == $branch->id ? 'selected' : '' }}>
                                                {{ $branch->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select select2-single @error('custodian_id') is-invalid @enderror" 
                                        ...
                                        @foreach($users as $user)
                                            <option value="{{ $user->id }}" {{ old('custodian_id', $unit->custodian_id) == $user->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select select2-single @error('supervisor_id') is-invalid @enderror" 
                                        ...
                                        @foreach($users as $user)
                                            <option value="{{ $user->id }}" {{ old('supervisor_id', $unit->supervisor_id) == $user->id ? 'selected' : '' }}>
                                                {{ $user->name }} @if($user->email) - {{ $user->email }} @endif
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" 
                                               ...
                                               id="float_amount" 
                                               name="float_amount" 
                                               value="{{ old('float_amount', $unit->float_amount) }}" 
                                               step="0.01" 
                                               min="0" 
                                               required>
                                        @error('float_amount')
                                    ...
                                    <small class="text-muted">Initial cash amount for this petty cash unit</small>
                                </div>
                            </div>

                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label for="maximum_limit" class="form-label">Maximum Limit</label>
                                    <div class="input-group">
                                        <span class="input-group-text">TZS</span>
                                        <input type="number" 
                                               ...
                                               id="maximum_limit" 
                                               name="maximum_limit" 
                                               value="{{ old('maximum_limit', $unit->maximum_limit) }}" 
                                               step="0.01" 
                                               min="0">
                                        @error('maximum_limit')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                    ...
                                    <small class="text-muted">Maximum allowed balance (optional)</small>
                                </div>
                            </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" 
                                               ...
                                               value="{{ old('approval_threshold', $unit->approval_threshold) }}" 
                                               step="0.01" 
                                               min="0">
                                    ...
                                    <small class="text-muted">Expenses above this amount require supervisor approval</small>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select select2-single @error('petty_cash_account_id') is-invalid @enderror" 
                                        ...
                                        @foreach($pettyCashAccounts as $account)
                                            <option value="{{ $account->id }}" {{ old('petty_cash_account_id', $unit->petty_cash_account_id) == $account->id ? 'selected' : '' }}>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select select2-single @error('bank_account_id') is-invalid @enderror" 
                                        ...
                                        @foreach($bankAccounts as $bankAccount)
                                            <option value="{{ $bankAccount->id }}" {{ old('bank_account_id', $unit->bank_account_id) == $bankAccount->id ? 'selected' : '' }}>
                    ...
                    <div class="form-section">
                        <h6 class="form-section-title">
                            <i class="bx bx-toggle-left me-2"></i>Status
                        </h6>
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <small class="text-muted">Enable or disable this petty cash unit</small>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('notes') is-invalid @enderror" 
                                              ...
                                              name="notes" 
                                              rows="4" 
                                              placeholder="Enter any additional notes or instructions for this petty cash unit...">{{ old('notes', $unit->notes) }}</textarea>
                                    @error('notes')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="col-12">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <button type="submit" class="btn btn-primary">
                                    <i class="bx bx-save me-1"></i>Update Petty Cash Unit
                                </button>
                            </div>
                        </div>
                    </div>
