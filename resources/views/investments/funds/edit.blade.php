...
<div class="page-wrapper">
    <div class="page-content">
        <x-breadcrumbs-with-icons :links="[
            ...
            ['label' => 'Edit Fund', 'url' => '#', 'icon' => 'bx bx-edit']
        ]" />

        <div class="d-flex justify-content-between align-items-center mb-3">
            <h6 class="mb-0 text-uppercase">EDIT UTT FUND</h6>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('investments.funds.update', \Vinkla\Hashids\Facades\Hashids::encode($fund->id)) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="fund_name" name="fund_name" value="{{ old('fund_name', $fund->fund_name) }}" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="fund_code" class="form-label">Fund Code / Symbol <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="fund_code" name="fund_code" value="{{ old('fund_code', $fund->fund_code) }}" required>
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="currency" name="currency" required>
                                    <option value="TZS" {{ old('currency', $fund->currency) == 'TZS' ? 'selected' : '' }}>TZS</option>
                                    <option value="USD" {{ old('currency', $fund->currency) == 'USD' ? 'selected' : '' }}>USD</option>
                                    <option value="EUR" {{ old('currency', $fund->currency) == 'EUR' ? 'selected' : '' }}>EUR</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="investment_horizon" name="investment_horizon" required>
                                    <option value="SHORT-TERM" {{ old('investment_horizon', $fund->investment_horizon) == 'SHORT-TERM' ? 'selected' : '' }}>Short-Term</option>
                                    <option value="LONG-TERM" {{ old('investment_horizon', $fund->investment_horizon) == 'LONG-TERM' ? 'selected' : '' }}>Long-Term</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="expense_ratio" name="expense_ratio" value="{{ old('expense_ratio', $fund->expense_ratio) }}" step="0.0001" min="0" max="100">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="branch_id" name="branch_id">
                                    ...
                                    @foreach($branches as $branch)
                                        <option value="{{ $branch->id }}" {{ old('branch_id', $fund->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="notes" name="notes" rows="3">{{ old('notes', $fund->notes) }}</textarea>
                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select select2-single" id="investment_account_id" name="investment_account_id" required>
                                            ...
                                            @foreach($assetAccounts as $account)
                                                <option value="{{ $account->id }}" {{ old('investment_account_id', $fund->investment_account_id) == $account->id ? 'selected' : '' }}>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select select2-single" id="income_account_id" name="income_account_id" required>
                                            ...
                                            @foreach($incomeAccounts as $account)
                                                <option value="{{ $account->id }}" {{ old('income_account_id', $fund->income_account_id) == $account->id ? 'selected' : '' }}>
                                                    {{ $account->account_code }} - {{ $account->account_name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select select2-single" id="loss_account_id" name="loss_account_id" required>
                                            ...
                                            @foreach($expenseAccounts as $account)
                                                <option value="{{ $account->id }}" {{ old('loss_account_id', $fund->loss_account_id) == $account->id ? 'selected' : '' }}>
                                                    {{ $account->account_code }} - {{ $account->account_name }}
                                                </option>
                                            @endforeach
                                        </select>
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="bx bx-save me-1"></i> Update Fund
                        </button>
                    </div>
                </form>
            </div>
