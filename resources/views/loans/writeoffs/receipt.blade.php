...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <!-- Write-off Information Card -->
        <div class="row mb-4">
            <div class="col-12">
                <div class="card border-primary">
                    <div class="card-header bg-light">
                        <h6 class="mb-0 fw-bold">
                            <i class="bx bx-info-circle me-2"></i>Write-off Information
                        </h6>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-3">
                                ...
                                @if($writeoff->loan)
                                    <a href="{{ route('loans.show', \Vinkla\Hashids\Facades\Hashids::encode($writeoff->loan_id)) }}" class="text-primary text-decoration-none">
                            ...
                            <div class="col-md-3">
                                ...
                                <span>{{ optional($writeoff->customer)->name ?? 'N/A' }}</span>
                            </div>
                            <div class="col-md-3">
                                <strong>Written-off Amount:</strong><br>
                                <span class="text-success">TZS {{ number_format($writeoff->outstanding, 2) }}</span>
                            </div>
                            <div class="col-md-3">
                                <strong>Type:</strong><br>
                                <span class="badge bg-warning text-dark">{{ ucfirst($writeoff->writeoff_type) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex align-items-center">
                    <div>
                        <h5 class="mb-0 text-white">
                            <i class="bx bx-receipt me-2"></i>Create Recovery Receipt
                        </h5>
                        <p class="mb-0 opacity-75">Create a receipt voucher for write-off recovery</p>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <form id="receiptVoucherForm" action="{{ route('loans.writeoffs.receipt.store', $writeoff) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                                    ...
                                    <div class="row">
                                        <div class="col-lg-4">
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select select2-single @error('payee_type') is-invalid @enderror"
                                                    id="payee_type" name="payee_type" required>
                                                    <option value="">-- Select Payee Type --</option>
                                                    <option value="customer" {{ old('payee_type', 'customer') == 'customer' ? 'selected' : '' }}>Customer</option>
                                        ...
                                        <div class="col-lg-8" id="customerSection">
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select select2-single @error('customer_id') is-invalid @enderror"
                                                    id="customer_id" name="customer_id">
                                                    <option value="">-- Select Customer --</option>
                                                    @foreach($customers as $customer)
                                                        <option value="{{ $customer->id }}" {{ old('customer_id', $writeoff->customer_id) == $customer->id ? 'selected' : '' }}>
                                        ...
                                        <div class="col-lg-8" id="otherPayeeSection" style="display: none;">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control @error('payee_name') is-invalid @enderror"
                                                    id="payee_name" name="payee_name" value="{{ old('payee_name') }}"
                        ...
                        <div class="col-12">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror"
                                    ...
                                    placeholder="Enter transaction description">{{ old('description', 'Loan write-off recovery - ' . ($writeoff->loan->loanNo ?? 'Loan #' . $writeoff->loan_id)) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="d-flex justify-content-start">
                                <a href="{{ route('loans.writeoffs.show', $writeoff) }}" class="btn btn-secondary me-2">
...
@push('scripts')
    <script>
        $(document).ready(function () {
            ...
            // Add default line item with provision income account
            @if(isset($defaultLineItem) && $defaultLineItem)
                addLineItem({{ $defaultLineItem->chart_account_id }}, {{ $defaultLineItem->amount }}, '{{ $defaultLineItem->description }}');
                calculateTotal();
            @else
